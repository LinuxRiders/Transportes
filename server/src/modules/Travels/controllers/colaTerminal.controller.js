...
export const createCola = async (req, res, next) => {
    try {
        const { id_terminal, id_vehiculo, hora_llegada, estado, created_by } = req.body;
...
export const updateCola = async (req, res, next) => {
    try {
        ...
        await ColaTerminal.update(req.params.id, fields, req.body.updated_by);
        const cola = await ColaTerminal.findById(req.params.id);
...
export const deleteCola = async (req, res, next) => {
    try {
        await ColaTerminal.softDelete(req.params.id, req.body.updated_by);
